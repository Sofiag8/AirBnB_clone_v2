...
env.hosts = ['35.196.158.6 web-01', '35.227.106.6 web-02']
...
def do_deploy(archive_path):
    ...
    try:
        ...
        run("sudo mkdir -p {}".format(folder_to_compress))
        #  uncompress archive to specific folder using the "-C" option
        #  tar -xvzf abc.tar.gz -C /opt/folder/
        #  tar compresses multiple files
        run("sudo tar -xvzf /tmp/{} -C {}".format(file_name,
                                                 folder_to_compress))
        #  Delete the archive from the web server
        run("sudo rm /tmp/{}".format(file_name))
        run("sudo mv {}/web_static/* {}/".format(folder_to_compress,
                                                 folder_to_compress))
        run("sudo rm -rf {}/web_static".format(folder_to_compress))
        #  delete symbolic link
        run("sudo rm -rf /data/web_static/current")
        #  create a new symbolic link  ln -s /usr/local/var /var/run
        #  on the server, create a linked with the current
        run("sudo ln -s {} /data/web_static/current".format(folder_to_compress))
        return True
    except:
        return False
